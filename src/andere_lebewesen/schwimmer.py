...
class Schwimmer(object):
    ...
    def create_new_schwimmer(self):
        if num_life >= 1:
            new_life = Schwimmer((self.position[0],self.position[1] ),"blue",random.randint(1000,20000),True,False,random.randint(200,300))
    ...
    def schwimmer_create(self):
        ...
        for i in range(self.startwert,self.endwert):
                self.body_1 = pygame.draw.rect(screen,self.body_colour,(self.position[0] + self.abstand * i,self.position[1] - self.abstand * i,self.block,self.block))

        for i in range(self.startwert,self.endwert) :
                self.body_2 = pygame.draw.rect(screen,self.body_colour,(self.position[0] - self.abstand * i,self.position[1] - self.abstand * i,self.block,self.block))

        for i in range(self.startwert,self.endwert):
                self.body_3 = pygame.draw.rect(screen,self.body_colour,(self.position[0] - self.abstand * i,self.position[1] + self.abstand * i,self.block,self.block))

        for i in range(self.startwert,self.endwert):
                self.body_4 = pygame.draw.rect(screen,self.body_colour,(self.position[0] + self.abstand * i,self.position[1] + self.abstand * i,self.block,self.block))
...
#schwimmer_family = [Schwimmer((random.randint(0,width // cell_size ),random.randint(0,height // cell_size)),) for _ in range(1)]
...
s = Schwimmer((random.randint(0,width ),random.randint(0,height )),"blue",random.randint(1000,20000),True,True,random.randint(200,300))
